...
<<echo=FALSE, results='hide'>>=
Diff = 100*(mean(result[,2]) - mean(result[,1])) / mean(result[,1])
@
 
On average we are doing \textbf{\Sexpr{round(Diff,digits = 3)}} \% less checks than Wil. \\
...
<<echo=FALSE, results='hide'>>=
Diff = 100*(mean(result[,5]) - mean(result[,4])) / mean(result[,4])
@
 
On average our overprocessing is \textbf{\Sexpr{round(Diff,digits = 3)}} \% less than Wil. \\
...
tt[1,] = round(table(dat$nr_checks_our)/1)
tt[2,] = round(table(dat$nr_checks_Wil)/1)
tt[3,] = round(table(dat$nr_checks_rand)/1)
